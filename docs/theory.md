...
当前解释器从 Minecraft JAR 内的原版 JSON 构建池、权重、数量提供器和函数序列；随机附魔的兼容性过滤与复杂等级附魔继续委托原版注册表/`EnchantmentHelper`。测试会把七张支持表（远古城市一张、堡垒四张、沙漠神殿一张、林地府邸一张）与原版 LootTable 对同一批 seed 比较完整物品序列和数量。这曾实际发现并修复一个边界：原版 `Mth.nextInt(min,max)` 在 `min == max` 时不推进 RNG。
