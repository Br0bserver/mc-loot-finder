...
版本层现在由 `StructureSpec` 描述：结构 ID、维度、random-spread 类型与参数、装饰 RNG step/index、共享结构集权重、箱子扫描后端、战利品表和默认目标。主世界/下界运行时与 CLI 管线由四种结构共用。远古城市和堡垒保留经过对拍的 Jigsaw 快速路径；通用正确性后端直接执行原版 `StructureStart.placeInChunk`，记录它创建的容器，目前供沙漠神殿和林地府邸使用。增加同类结构通常只需配置，而不再写一套箱子坐标逻辑；碰到新的世界交互时才扩展记录世界。

战利品解释器直接读取 26.1.2 原版数据包 JSON，并只实现、验证这七张表实际用到的函数子集：
...
- 四种 random-spread 参数（含林地府邸 triangular 分布）与原版实现对拍；
...
- 七张支持表各 2048 个 seed 与原版 LootTable 比较完整物品序列和数量。
