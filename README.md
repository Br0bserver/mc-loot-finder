...
`mc-loot-finder` 是一个用于 Minecraft Java 26.1.2 的命令行工具。当前分支已按独立
Java 26.1.2 oracle 恢复固定向量，并通过 Linux/Windows Rust CI、行为 smoke 和
固定 SteelMC release 的独立 parity probe；目标结构的精确扫描已验证，但本分支仍不
发布正式 Release。
